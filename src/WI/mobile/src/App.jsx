      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-6">
        {recentVolumes.length > 0 && (
          <section>
            ...
            <div className="space-y-2">{recentVolumes.map(db => <div key={db} onClick={() => { setSelectedDb(db); setTab('explorer'); }} className={`flex items-center gap-3 px-4 py-3 rounded-xl border btn-touch ${selectedDb === db ? 'bg-[#3bb5ff]/15 border-[#3bb5ff]' : 'bg-[#0f1f3a]/40 border-[#1a3a5c]'}`}><span className={selectedDb === db ? 'text-[#3bb5ff]' : 'text-gray-500'}>{Ico.cube}</span><div className="text-sm font-bold truncate text-white flex-1">{db.replace('.db', '')}</div><button onClick={(e) => { e.stopPropagation(); setRecentVolumes(prev => { const u = prev.filter(d => d !== db); localStorage.setItem('mob_recentVolumes', JSON.stringify(u)); return u; }); setExternalVolumes(prev => prev.filter(p => p !== db)); localStorage.setItem('mob_externalVolumes', JSON.stringify(externalVolumes.filter(p => p !== db))); setDbs(prev => prev.filter(p => p !== db)); if (selectedDb === db) setSelectedDb(''); showToast('Removed', 'success'); }} className="p-1.5 text-gray-600 hover:text-red-400 rounded-lg transition-colors flex-shrink-0">{Ico.close}</button></div>)}</div>
          </section>
        )}
        <section>
