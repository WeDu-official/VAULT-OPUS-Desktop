...
export default function SeeVersionsModal({ itemPath, onClose, onVersionSelect }) {
    ...
    const handleVersionDoubleClick = (version) => {
        if (onVersionSelect) {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-200">
            <div className="bg-[#0a1628] border border-[#1a3a5c] rounded-xl shadow-2xl w-full max-w-lg max-h-[70vh] overflow-hidden animate-in zoom-in-95 duration-200">
                        ...
                        <div className="flex-1 overflow-y-auto">
                            ...
                            <div className="space-y-1">
                                {versions.map((version, idx) => (
                                    <div
                                        key={idx}
                                        onDoubleClick={() => handleVersionDoubleClick(version)}
                                        className="flex items-center justify-between px-3 py-2 bg-[#0f1f3a] border border-[#1a3a5c] rounded cursor-pointer hover:bg-[#1a3a5c] hover:border-[#3bb5ff]/50 transition-all duration-150 group active:scale-[0.98]"
                                        title="Double-click to view this version"
                                    >
                                        <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-xs text-[#3bb5ff]/50 group-hover:text-[#3bb5ff]/80 transition-colors">
                ...
                {/* Footer */}
                <div className="p-3 border-t border-[#1a3a5c] bg-[#0a1628]">
                    <div className="text-[10px] text-gray-500 text-center">
                        Double-click on a version to view its contents
                    </div>
                </div>
            </div>
        </div>
